...
async def run_bot():
    ...
    await bot.load()
    log.info(f"SuggestionsBot indítása | %s", bot.version)
    print(f"SuggestionsBot indítása | {bot.version}", bot.version)
    print("Pordban vagyunk" if bot.is_prod else "Indítjuk a nem pordos verziót")
