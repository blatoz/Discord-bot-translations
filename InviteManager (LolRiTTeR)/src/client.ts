...
export class IMClient extends Client {
	...
	public async waitForStartupTicket() {
		...
		const interval = setInterval(
			() => console.log(`Várakozás a ticketre ${chalk.blue(Math.floor(process.uptime() - start))} másodperce...`),
			10000
		);
	...
	private async onClientReady(): Promise<void> {
		if (this.hasStarted) {
			console.error('A BOT MÁR ELINDULT, IGNORÁLVA A READY EVENTET');
			return;
		}
		...
		console.log(chalk.green(`Kliens elindult! Szerverek száma: ${chalk.blue(this.guilds.size)}`));
		...
		this.guilds.forEach(async (guild) => {
			...
			if (bannedGuild) {
				...
				await dmChannel
					.createMessage(
						`Szia! Köszi hogy meghívtál a \`${guild.name}\` szerverre!\n\n` +
							'Úgy tűnik, hogy ezt a szervert eltiltották az InviteManager bot használatától..\n' +
							'Ha úgy gondolod, hogy ez hiba, kérlek lépj kapcsolatba a személyzettel a támogató szerveren	.\n\n' +
							`${this.config.bot.links.support}\n\n` +
							'Jegyzet: A szerverek szűrése miatt most már nem tudod hozzáférni a szerverhez.'
					)
					.catch(() => undefined);
				await guild.leave();
				return;
					...
					if (!premium) {
						...
						if (!premium) {
							...
							await dmChannel
								.createMessage(
									'Szia!' +
										`Köszi hogy meghívtál a \`${guild.name}\` szerverre!\n\n` +
										'Csak a pro verzió érhető el a Patreon támogatásával.\n\n' +
										'Foglalj támogatást itt: https://www.patreon.com/invitemanager\n\n' +
										'Ha költötted támogatást, akkor futtasd a következő parancsokat a szerveren:\n' +
										'`!premium check` és `!premium activate`\n\n' +
										'Jegyzet: A szerverek szűrése miatt most már nem tudod hozzáférni a szerverhez.'
								)
								.catch(() => undefined);
	...
	public serviceStartupDone(service: IMService) {
		...
		if (this.startingServices.length === 0) {
			console.log(chalk.green(`Minden szolgáltatás elindult`));
	...
	private async onGuildCreate(guild: Guild): Promise<void> {
		...
		channel
			.createMessage(
				`Szia! Köszi hogy meghívtál a \`${guild.name}\` szerverre!\n\n` +
					'Jegyzet: A szerverek szűrése miatt most már nem tudod hozzáférni a szerverhez.\n\n' +
					'Ha szeretnél beállítani a beérkezést, akkor futtasd a következő parancsot:\n' +
					'`!setup`\n\n' +
					'Ha szeretnél segíteni, akkor futtasd a következő parancsot:\n' +
					'`!help`\n\n' +
					`Ennyi lenne! Élvezze a botot, és ha bármilyen kérdése van, csatlakozzon nyugodtan a támogatási szerverünkhöz!\n` +
					'https://discord.gg/szervered meghívó kódja'
			)
			.catch(() => undefined);
	}
	...
	private async onGuildMemberAdd(guild: Guild, member: Member) {
		...
		if (member.user.bot) {
			...
			if (this.type === BotType.regular && member.user.id === this.config.bot.ids.pro) {
				console.log(`BOT LETILTÁSA ${guildId} SZEVEREN, MERT A PRO VERZIÓ BELÉPETT`);
	...
	private async onGuildMemberRemove(guild: Guild, member: Member) {
		...
		if (this.type === BotType.regular && member.user.id === this.config.bot.ids.pro) {
			...
			console.log(`BOT ENGEDÉLYEZÉSE ${guildId} SZEVEREN, MERT A PRO VERZIÓ KILÉPETT`);
		}
	}
	...
	public async setActivity() {
		...
		const name = this.settings.activityMessage || `weboldalad címe`;
	...
	private async onGuildUnavailable(guild: Guild) {
		console.error('DISCORD SZERVER NEM ELÉRHETŐ:', guild.id);
	}

	private async onWarn(warn: string) {
		console.error('DISCORD HIBÁSÁG:', warn);
		this.stats.wsWarnings++;
	}

	private async onError(error: Error) {
		console.error('DISCORD HIBA:', error);
		this.stats.wsErrors++;
	}
