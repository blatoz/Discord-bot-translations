    ...
    async run() {
        for (const result of await database.queryAll('SELECT * FROM moderations WHERE action = \'mute\' AND active = TRUE AND expireTime IS NOT NULL AND expireTime <= ?',
            ...
            try {
                ...
                try {
                    await member.unmute('Ideiglenes némitás megcsinálva!', null, bot.client.user);
                }
                catch (e) {
                    if (e.code === RESTJSONErrorCodes.MissingPermissions) {
                        ...
                        await guild.log(new ErrorEmbed('Hiányzó jogosultságok felhasználó némitás feoldása közben!')
                            .setAuthor({name: await member.displayAvatarURL(), iconURL: await member.displayAvatarURL()})
                            .setFooter({text: user.id})
                            .toMessage(false));
                        await logger.warn(`Hiányzó jogosutlságok hogy némitást feloldjam felhasználónak: ${result.userid} szerveren ${result.guildid}`);
                    } else {
                        throw e;
                    }
                }
            } catch (e) {
                await logger.error(`Nem sikerült feloldani a(z) ${result.userid} felhasználó némitását feoldani a(z) ${result.guildid} szerverben.`, e);
            }
        }
    }
}
