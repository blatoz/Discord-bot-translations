...
metadata: {
    name: "calculate",
    description: "Ellenőrizze, mennyi XP-re van szüksége egy bizonyos szint eléréséhez.",
    args: [
        { type: "integer", name: "target", description: "A kívánt szint", min: 1, max: 1000, required: true },
        { type: "user", name: "member", description: "Melyik tagnak", required: false }
    ]
},
...
async run(client, int, tools) {
    ...
    if (db.settings.rankCard.disabled) {
        let miniEmbed = tools.createEmbed({
            title: `Szint ${tools.commafy(targetLevel)}`,
            color: cardCol || member.displayColor || await member.user.fetch().then(x => x.accentColor),
            description: `${tools.commafy(targetXP)} XP kell`,
            footer: "A rangkártyák le vannak tiltva, ezért a számítások rejtve vannak!"
        })
    ...
    let levelDetails = [
        `**Jelenlegi XP: **${tools.commafy(xp)} (Level ${tools.commafy(userLevel)})`,
        `**Cél XP: **${tools.commafy(targetXP)}`,
        `**Maradt XP: **${reached? "0 (" : ""}${tools.commafy(targetXP - xp)}${reached ? ")" : ""}`
    ]

    if (!reached) levelDetails = levelDetails.concat([
        "",
        `**XP per üzenet: **${db.settings.gain.min == db.settings.gain.max ? tools.commafy(Math.round(db.settings.gain.min * multiplier)) : `${tools.commafy(Math.round(db.settings.gain.min * multiplier))} - ${tools.commafy(Math.round(db.settings.gain.max * multiplier))}`}`,
        `**Üzenet maradt: **${estimatedRange}`,
        `**Cooldown maradt: **${estimatedTime == Infinity ? "Az idők végezetéig" : tools.time(estimatedTime * 1000, 1)}`,
    ])

    let embed = tools.createEmbed({
        author: { name: member.user.displayName, iconURL: member.displayAvatarURL() },
        title: `Level ${tools.commafy(targetLevel)}${reached ? " (elérve!)" : ""}`,
        color: cardCol || member.displayColor || await member.user.fetch().then(x => x.accentColor),
