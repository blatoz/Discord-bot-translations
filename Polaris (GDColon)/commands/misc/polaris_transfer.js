...
module.exports = {
    ...
    async run(client, serverID, importSettings={}, guilds) {
        ...
        if (!foundServer) return { error: "Nem vagyok bent a szerveren" }
        else if (foundServer && !foundServer.owner) return { error: "Nem a tulajdonosa" }
        else if (foundServer.id == serverID) return { error: "Nem lehet átvinni a szerver adatait." }

        let toTransfer = []
        if (importSettings.xp) toTransfer.push("users")
        if (importSettings.settings) toTransfer.push("settings")
        if (!toTransfer.length) return { error: "Érvénytelen importálási opciók!" }

        let details = []
        let importedUsers = 0

        let transferData = await client.db.fetch(transferFrom, toTransfer)
        if (!transferData) return { error: `Nincsen Polaris adat ezzen a szerveren: ${foundServer.name}`, code: "invalidImport" }
