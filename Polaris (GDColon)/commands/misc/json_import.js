...
module.exports = {
    ...
    async run(client, serverID, importSettings={}, jsonData) {
        ...
        if (jsonData.users && importSettings.xp) {
            ...
            if (!importSettings.isDev && userEntries.length > 2000) return { error: "Csak 2000 felhasználóig lehet importálni, csak ha nem a fejesztő vagy." }
        ...
        if (!details.length) return { error: `Nem található json adat! A szintaxis: { users: {...} }, settings: {...} }` }
