...
async run(client, int, tools, selected) {
    ...
    if (!entries) return tools.warn("Érvénytelen kategória!")

    let rows = []
    let options = []
    let groupName = "Beállítások"
    ...
    let embed = tools.createEmbed({
        ...
        footer: isHome ? "A alapértelmezett beállítások itt módosíthatók" : null
    })

    let dropdown = new Discord.StringSelectMenuBuilder()
    .setCustomId(`configmenu_${int.user.id}`)
    .setPlaceholder(isHome ? "Válassz kategóriát..." : "Válassz beállítást...")
    .addOptions(...options)
