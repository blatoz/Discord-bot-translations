...
async run(client, int, tools, selected) {
    ...
    let footer = data.tip || ""
    if (setting.default !== undefined) footer += `${footer ? "\n" : ""}Alapértemezett: ${previewSetting(setting.default)}`

    let embed = tools.createEmbed({
        color: tools.COLOR,
        title: data.name,
        description: `**Jelenlegi érték:** ${currentVal}\n\n💡 ${data.desc}`,
        footer: footer || null
    })

    let buttons = tools.button([
        {style: "Secondary", label: "Vissza", customID: `settings_list~${group}~${int.user.id}`},
        {style: "Primary", label: (setting.type == "bool") ? "Váltás" : "Szerkesztés", customId: `settings_edit~${settingID}~${int.user.id}`}
    ])
