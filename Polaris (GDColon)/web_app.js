...
app.get("/api/guilds", async function(req, res) {
    ...
    if (!user || !guilds) return res.apiError("Nincsen bejelentlezve!", "login")
...
app.get("/api/settings/:id", async function(req, res) {
    let [user, guilds] = await getDiscordInfo(req)
    if (!user || !guilds) return res.apiError("Nincsen bejelentkezve!", "login")
    let serverID = req.params.id
    if (!serverID) return res.apiError("Érvénytelen szerver ID!")
    ...
    if (!foundGuild && !force) return res.apiError("Hiányzó jogosultságok!", "noPerms")

    let canManage = force || canManageServer(foundGuild)
    if (!canManage) return res.apiError("Hiányzó jogosultságok!", "noPerms")

    let serverData = await client.db.fetch(serverID, ["settings", "info"]) || await client.db.create({ _id: serverID })
    if (!serverData) return res.apiError("Nincsen szerver adat!");
    ...
    if (!guildData) return res.apiError("Nem sikerült letölteni a szerver adatait!");
...
app.post("/api/settings", async function(req, res) {
    ...
    if (typeof req.body != "object") return res.apiError("Érvénytelen mentési adat!");
    let guildID = req.body.guildID
    if (!guildID) return res.apiError("Nem egy szerver ID!");

    let [user, guilds] = await getDiscordInfo(req)
    if (!user || !guilds) return res.apiError("Nincsen bejelentkezve!")

    let force = tools.isDev(user);

    let foundGuild = guilds.find(x => x.id == guildID)
    if (!foundGuild && !force) return res.apiError("Nincsen bent a szerveren!")

    let canManage = canManageServer(foundGuild)
    if (!canManage && !force) return res.apiError("Szerver kezelése jogodultdág kötelező!")

    if (req.body.resetSettings) {
        return client.db.update(guildID, { $unset: { "settings": "x" }, $set: { "info.lastUpdate": Date.now() } }).exec().then(() => {
            return res.end("Összes beállítás visszaállítva!")
        }).catch(console.error)
    }

    else if (req.body.resetXP) {
        return client.db.update(guildID, { $unset: { "users": "x" } }).exec().then(() => {
            return res.end("Összes Xp vissza lett állítva!")
        }).catch(console.error)
    }
