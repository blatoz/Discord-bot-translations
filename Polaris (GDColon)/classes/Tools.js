...
class Tools {
    constructor(client, int) {
        ...
        this.errors = {
            xpDisabled: `XP nincsen engedélyezve!${this.canManageServer() ? ` (engedélyezhető ${this.commandTag("config")})` : ""}`,
            noData: "Ez a szerver még nem tartalmaz adatokat!",
            noBotXP: "A botok nem szerezhetnek XP-t!",
            cantManageRoles: "Nincs jogosultságom a szerepkörök kezelésére!",
            notMod: "Nincs jogosultsága ennek a parancsnak a használatára!"
        }
        ...
        this.noXPYet = function(user) {
            return this.warn(user.bot ? "*noBotXP" : user.id != int.user.id ? `${user.displayName} nincsen xpje!` : `Nincsen xpje!`)
        }
        ...
        this.confirmButtons = function(titleText, titleColor, cancelText, cancelColor) {
            return this.button([
                {style: titleColor || "Success", label: titleText || 'Megerősítés', customId: 'confirm'},
                {style: cancelColor || "Danger", label: cancelText || 'Mégsem', customId: 'cancel'}
            ])
        }
        ...
        this.time = function(ms, decimals=0, noS, shortTime) {
            ...
            if (seconds < 1) return timeFormat((ms / 1000).toFixed(2), shortTime ? "ms" : "másodperc")
            if (seconds < 60) return timeFormat(seconds, shortTime ? "ms" : "másodperc")
            else if (minutes < 60) return timeFormat(minutes, shortTime ? "p" : "perc")
            else if (hours <= 24) return timeFormat(hours, "óra")
            else if (days <= 365) return timeFormat(days, "nap")
            else return timeFormat(years, "év")
        }
        ...
        this.jsonImport = function(serverID, url, xpKey="xp", idKey="id",) {
            fetch(url).then(res => res.json()).then(list => {
                ...
                client.db.update(serverID, { $set: { users } }).exec().then(() => { int.channel.send({ content: "Sikeres!" }) })
            }).catch(e => { int.channel.send({ content: "Sikertelen! " + e.message }) })
        }
