...
if (!client.shard) {
    console.error("Nincsen shard infó!\nGyőződjön meg róla, hogy a botot a következő helyről indítja el polaris.js, nem a index.js fájlról!\nHa ez nem segít, akkor a Discord.js verziója régi, frissítse a legújabbra!")
    return process.exit()
}
...
// when online
client.on("ready", () => {
    if (client.shard.id == client.shard.count - 1) console.log(`Bot online! (${+process.uptime().toFixed(2)} másodperc)`)
    ...
    .then(cmds => {
        if (cmds.size < 1) { // no commands!! deploy to test server
            console.info("!!! Nem található globális parancs, a fejlesztői parancsok telepítése a tesztkiszolgálóra (A globális parancsok telepítéséhez használja a /deploy global=true parancsot.)")
...
client.on("interactionCreate", async int => {
    ...
    if (!int.guild) return int.reply("Nem használhatsz parancsokat DM-ekben!")
    ...
    // dev perm check
    if (foundCommand.metadata.dev && !tools.isDev()) return tools.warn("Csak a fejlesztők használhatják!")
    else if (config.lockBotToDevOnly && !tools.isDev()) return tools.warn("Csak a fejlesztők használhatják!")

    try { await foundCommand.run(client, int, tools) }
    catch(e) { console.error(e); int.reply({ content: "**Hiba!** " + e.message, ephemeral: true }) }
})
